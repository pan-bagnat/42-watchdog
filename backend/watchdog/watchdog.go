...
func postApprenticesAttendancesLocked() {
	...
	if atLeastOneField {
		...
		subject := fmt.Sprintf("Watchdog – Daily Report - %s", time.Now().Format("02/01/2006"))
		if len(recipients) == 0 {
			Log(fmt.Sprintf("[WATCHDOG] [MAIL] skipped: no recipients configured for daily report %q", subject))
		} else if err := mailer.Send(recipients, subject, htmlBody.String(), true); err != nil {
			Log(fmt.Sprintf("[WATCHDOG] [MAIL] failed: subject=%q recipients=%d error=%v", subject, len(recipients), err))
		} else {
			Log(fmt.Sprintf("[WATCHDOG] [MAIL] sent: subject=%q recipients=%d", subject, len(recipients)))
		}
	} else {
		Log("[WATCHDOG] [MAIL] skipped: no apprentice rows to include in daily report")
	}
}
