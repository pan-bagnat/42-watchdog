...
func AdminStats(statuses []string, weekday int, restrictToPresenceWindow bool) (AdminStatsResponse, error) {
	...
	allowedLogins := make(map[string]AdminUserSummary, len(users))
	...
	for _, user := range users {
		...
		allowedLogins[login] = user
		typeCounts[coalesceManagedStatus(user.Status, statusFromSignals(user.IsApprentice, user.Profile))] += 1
	}

	response := AdminStatsResponse{
		SelectedWeekday:          weekday,
		RestrictToPresenceWindow: restrictToPresenceWindow,
		PresenceWindowStart:      "07:30",
		PresenceWindowEnd:        "20:30",
		UserTypeDistribution: []AdminStatsTypeCount{
			{Status: "apprentice", Count: typeCounts["apprentice"]},
			{Status: "student", Count: typeCounts["student"]},
			{Status: "pisciner", Count: typeCounts["pisciner"]},
			{Status: "staff", Count: typeCounts["staff"]},
			{Status: "extern", Count: typeCounts["extern"]},
		},
		FilteredUserCount: len(allowedLogins),
	}
	if len(allowedLogins) == 0 {
		response.AverageSeenByWeekday = zeroWeekdayBuckets()
		response.AveragePresenceByHour = zeroHourBuckets(restrictToPresenceWindow)
		response.AveragePresenceByWeekday = zeroWeekdayHourSeries(restrictToPresenceWindow)
		response.DoorUsage = []AdminStatsDoorCount{}
		return response, nil
	}
	if !restrictToPresenceWindow {
		response.PresenceWindowStart = "00:00"
		response.PresenceWindowEnd = "24:00"
	}

	rangeStartDayKey, rangeEndDayKey := adminStatsDayRange()
	scheduleAdminStatsHistoricalMonthFetches(allowedLogins, rangeStartDayKey, rangeEndDayKey)

	records := make(map[string]*adminStatsDayRecord)
	observedDays := make(map[string]struct{})
	doorUsage := make(map[string]int)

	if err := loadAdminStatsSummaryRanges(allowedLogins, records, observedDays, rangeStartDayKey, rangeEndDayKey); err != nil {
		return AdminStatsResponse{}, err
	}
	if err := loadAdminStatsCurrentRanges(allowedLogins, records, observedDays, rangeStartDayKey, rangeEndDayKey); err != nil {
		return AdminStatsResponse{}, err
	}
	if err := loadAdminStatsAttendanceBounds(allowedLogins, records, observedDays, rangeStartDayKey, rangeEndDayKey); err != nil {
		return AdminStatsResponse{}, err
	}
	if err := loadAdminStatsLocationRanges(allowedLogins, records, observedDays, rangeStartDayKey, rangeEndDayKey); err != nil {
		return AdminStatsResponse{}, err
	}
	if err := loadAdminStatsBadgeEvents(allowedLogins, records, observedDays, doorUsage, restrictToPresenceWindow, rangeStartDayKey, rangeEndDayKey); err != nil {
		return AdminStatsResponse{}, err
	}
	...
	hourBuckets := buildStatsHourBuckets(restrictToPresenceWindow)
	...
	for _, record := range records {
		start, end, ok := clipStatsRange(record.DayKey, record.First, record.Last, restrictToPresenceWindow)
		if !ok {
			continue
		}
...
func zeroHourBuckets(restrictToPresenceWindow bool) []AdminStatsBucket {
	buckets := buildStatsHourBuckets(restrictToPresenceWindow)
...
func zeroWeekdayHourSeries(restrictToPresenceWindow bool) []AdminStatsSeries {
	...
	for _, label := range weekdayLabels {
		series = append(series, AdminStatsSeries{
			Key:    strings.ToLower(label),
			Label:  label,
			Values: zeroHourBuckets(restrictToPresenceWindow),
		})
	}
	return series
}

func buildStatsHourBuckets(restrictToPresenceWindow bool) []statsHourBucket {
	day, _ := parseStatsDayKey("2000-01-03")
	if restrictToPresenceWindow {
		buckets := make([]statsHourBucket, 0, 13)
		start := time.Date(day.Year(), day.Month(), day.Day(), 7, 30, 0, 0, day.Location())
...
func loadAdminStatsBadgeEvents(allowed map[string]AdminUserSummary, records map[string]*adminStatsDayRecord, observedDays map[string]struct{}, doorUsage map[string]int, restrictToPresenceWindow bool, startDayKey, endDayKey string) error {
	if storageDB == nil {
		return nil
	}
	...
	for rows.Next() {
		...
		if statsTimeInWindow(dayKey, occurredAt, restrictToPresenceWindow) {
...
func statsWindowBounds(dayKey string, restrictToPresenceWindow bool) (time.Time, time.Time, bool) {
	...
	if restrictToPresenceWindow {
		return time.Date(day.Year(), day.Month(), day.Day(), 7, 30, 0, 0, day.Location()),
			time.Date(day.Year(), day.Month(), day.Day(), 20, 30, 0, 0, day.Location()),
			true
	}
...
func clipStatsRange(dayKey string, start, end time.Time, restrictToPresenceWindow bool) (time.Time, time.Time, bool) {
	if start.IsZero() || end.IsZero() {
		return time.Time{}, time.Time{}, false
	}
	if end.Before(start) {
	...
	windowStart, windowEnd, ok := statsWindowBounds(dayKey, restrictToPresenceWindow)
	if !ok {
		return time.Time{}, time.Time{}, false
	}
...
func statsTimeInWindow(dayKey string, value time.Time, restrictToPresenceWindow bool) bool {
	windowStart, windowEnd, ok := statsWindowBounds(dayKey, restrictToPresenceWindow)
	if !ok {
		return false
	}
	value = value.In(windowStart.Location())
	if restrictToPresenceWindow {
		return (value.Equal(windowStart) || value.After(windowStart)) && value.Before(windowEnd)
	}
	return (value.Equal(windowStart) || value.After(windowStart)) && value.Before(windowEnd)
}
